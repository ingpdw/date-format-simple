...
var DateFormat = (function () {
  ...
  _createClass(DateFormat, [{
    ...
    value: function toGMTDate(utc) {
      ...
      return new Date(date[0], parseInt(date[1], 10) - 1, date[2], time[0], parseInt(time[1], 10) - clientDate.getTimezoneOffset(), time[2].split('.')[0]);
    }
  }, {
    key: 'isToday',
    value: function isToday(year, month, day) {
      var current = this.toGMTDate(Conf.gmtDate);
      return !(new Date(current.getFullYear(), current.getMonth() + 1, current.getDate()) - new Date(year, month, day));
    }
  }, {
    key: 'getNow',
